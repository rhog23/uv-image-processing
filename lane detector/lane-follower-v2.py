...
class MyHandCodedLaneFollower(object):
    ...
    def follow_lane(self, frame):
        ...
        frame_lanes, new_steering_angle = pipeline_lane_detector(
...
def draw_patches(img, patch_):
    cv2.line(
        img,
        (patch_["x"][0], patch_["y"][0]),
        (patch_["x"][1], patch_["y"][0]),
        (0, 165, 255),
        1,
    )
    cv2.line(
        img,
        (patch_["x"][0], patch_["y"][1]),
        (patch_["x"][1], patch_["y"][1]),
        (0, 165, 255),
        1,
    )
    cv2.line(
...
def get_centroids_from_patches(list_lines, patch_):
    ...
    for line in list_lines:
        ...
        if (
            ...
            if (
                ...
                centroids["bottom"] = np.vstack(
                    (centroids["bottom"], np.array([x1, y1]))
                )
    ...
    if len(centroids["bottom"]) > 0:
        ...
        if centroids["bottom"][1] <= centroids["top"][1]:
            velocity = (
                centroids["bottom"][0] - centroids["top"][0],
                centroids["bottom"][1] - centroids["top"][1],
            )
        else:
            velocity = (
                centroids["top"][0] - centroids["bottom"][0],
    ...
    if lines is None:
        ...
        for patch in list_patch:
            ...
            if not empty_bool:
                if (
                    velocity[1] < -0.25 and centroids["bottom"][0] > 160
                ):  # velocity[0] < -0.25 and
        ...
        if len(X_right) > 1:
            ...
            cv2.line(
                img_bottom_half_bgr, (219, int(y1)), (319, int(y2)), (150, 50, 240), 5
            )  # 150,50,255
        ...
        if len(X_left) > 1:
            ...
            cv2.line(
                img_bottom_half_bgr, (0, int(y1)), (100, int(y2)), (150, 50, 240), 5
            )
        ...
        if len(X_right) > 1 and len(X_left) > 1:
            ...
            cv2.line(
                img_bottom_half_bgr,
                (int(np.clip(mid_star, -10000, 10000)), 25),
                (160, 100),
                (0, 0, 255),
                5,
            )
        ...
        if np.abs(mid_star - 160) < 2:
            ...
            steering_angle = 90 + np.degrees(np.arctan((mid_star - 160) / 75.0))
            steering_angle = np.clip(steering_angle, 55, 135)

        stable_steering_angle = stabilize_steering_angle(
            steering_angle, past_steering_angle
        )
